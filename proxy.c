...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		fprintf(stderr, "usage: %s [Handler port]\n", argv[0]);
		exit(1);
	}

	handleraddr = "localhost";
	agentaddr = "pastushkov.com";
	agentport = 4444;

	handlerport = atoi(argv[1]);

	assert(handleraddr);
	assert(handlerport > 0);
	assert(agentaddr);
	assert(agentport > 0);
	...
	if ((handler = open_remote_host(handleraddr, handlerport)) < 0) {
		close(agent);
		agent = -1;
		printf("Unable to connect to %s! (Did you start the handler?)\n",
				handleraddr);
		exit(-1);
	}
	printf("Connected to %s.\n", handleraddr);
	if ((agent = open_remote_host(agentaddr, agentport)) < 0) {
		close(handler);
		handler = -1;
	...
	syslog(LOG_INFO, "connected to %s:%d fd=%d", agentaddr, agentport, agent);
	syslog(LOG_INFO, "connected to %s:%d fd=%d", handleraddr, handlerport,
			handler);
	printf("Proxy started.");
	service_client(handler, agent);

	printf("Proxy session ended.\n");
	close(handler);
	handler = -1;
	close(agent);
	agent = -1;
